...
logging.basicConfig(filename='failure.log',level=logging.DEBUG,format='%(asctime)s - %(message)s', datefmt= '%d-%b-%y %H:%M:%S')
...
class bill_window:
    def __init__(self, top=None):
        ...
        self.img = PhotoImage(file="images/bill_window1.png")
    ...
    def remove_product(self):
        if not self.cart.isEmpty():
            self.Scrolledtext1.configure(state="normal")
            strr = self.Scrolledtext1.get('1.0', END)
            if strr.find('Total') == -1:
                try:
                    removed_item = self.cart.items.pop()  # Remove the last item from the cart
                    # Remove the last item from the products list
                    for i, product in enumerate(reversed(self.products)):
                        if product.product_name == removed_item.product_name:
                            del self.products[len(self.products) - 1 - i]  # Remove item from products list
                            break  # Exit loop once item is removed
                except IndexError:
                    messagebox.showerror("Oops!", "Cart is empty", parent=biller)
                else:
                    self.update_scrolled_text()  # Update the displayed text in the ScrolledText widget
            else:
                try:
                    removed_item = self.cart.items.pop()  # Remove the last item from the cart
                    # Remove the last item from the products list
    ...
    def update_scrolled_text(self):
        ...
        for item in self.products:
            bill_text = "{}\t\t\t\t\t\t{}\t\t\t\t\t   {}\n".format(item.product_name, item.qty, item.price)
    ...
    def total_bill(self):
        if self.cart.isEmpty():
            ...
            if strr.find('Total') == -1:
                ...
                total_str = "\nTotal\t\t\t\t\t\t\t\t\t\t\tRs. {}".format(total)
                self.Scrolledtext1.insert('insert', total_str)
                divider2 = "\n" + ("─" * 61)
                self.Scrolledtext1.insert('insert', divider2)
                self.Scrolledtext1.configure(state="disabled")

                template_path = "images/orgbill.png"
                customer_name = cust_name.get()
                bill_number = cust_new_bill.get()
                phone_number = cust_num.get()
                date1 = bill_date.get()
                selected_pr_name = self.combo3.get()
                pr_quantity = self.entry4.get()
                products = []
                for item in self.products:
                    product_name = item.product_name
                    product_qty = item.qty
                    products.extend(get_products(cur, product_name, product_qty))
                # Generate PDF bill with total
                generate_bill_with_fields(template_path, customer_name, bill_number, phone_number, date1, self.products,total)
        ...
        if self.state == 1:
            ...
            if(cust_name.get()==""):
                ...
                if strr.find('Total')==-1:
                    ...
                    bill_date.set(str(date_dt.today()))
    ...
    def clear_generated_pdf(self):
        # Define the path to the generated PDF file
        pdf_file_path = "bill_with_fields.pdf"
        ...
        if os.path.exists(pdf_file_path):
            ...
            print("PDF file does not exist or has already been deleted.")
    ...
    def clear_bill(self):
        ...
        # Clear the generated PDF file
        self.clear_generated_pdf()
    ...
    def print_pdf(pdf_path, printer_name):
        ...
        with open(pdf_path, 'rb') as file:
            ...
            # Set the printer to the specified printer name
            ...
            job_info = win32print.StartDocPrinter(printer_handle, 1, ("Document", None, "RAW"))
...
def generate_bill_with_fields(template_path, customer_name, bill_number, phone_number, date1, products, total_amount):
    # Create a canvas
    c = canvas.Canvas("bill_with_fields.pdf", pagesize=letter)

    # Load the bill template image
    template = ImageReader(template_path)

    # Add text to the fields on the first page
    c.drawImage(template, 0, 0, width=letter[0], height=letter[1])
    c.drawString(120, 607, customer_name)
    c.drawString(100, 575, bill_number)
    c.drawString(524, 607, phone_number)
    c.drawString(485, 575, date1)
    ...
    for product in products:
        # Check if there's enough space for another product, otherwise start a new page
        if y_position < 50:
            c.showPage()  # Start a new page
            y_position = 750  # Reset y_position for new page

        c.drawString(25, y_position, product.product_name)
        c.drawString(530, y_position, str(product.price))
        c.drawString(300, y_position, str(product.qty))
        y_position -= 20  # Move to the next row for the next product

    # Set line width and color for the divider
    c.setLineWidth(2)  # Increase line thickness
    c.setFillColor(black)  # Set color to black

    # Draw divider
    c.line(25, y_position, 590, y_position)  # Draw a line from left to right

    # Draw total amount
    y_position -= 20
    c.drawString(25, y_position, "Total:")
    c.drawString(530, y_position, str(total_amount))
...
def get_products(cur, selected_pr_name, pr_quantity):
    products = []
    find_mrp = "SELECT mrp, stock FROM raw_inventory WHERE product_name = ?"
    cur.execute(find_mrp, [selected_pr_name])
    results = cur.fetchall()
    ...
    for result in results:
        ...
        if str(pr_quantity).isdigit() and (stock - int(pr_quantity)) >= 0:
            sp = mrp * int(pr_quantity)
            product = Item(selected_pr_name, sp, int(pr_quantity))
